#ifndef NATIVE_SURFACE_UTILS_H // !NATIVE_SURFACE_UTILS_H
...
#define ResolveMethod(ClassName, MethodName, Handle, MethodSignature)          \
  ClassName##__##MethodName =                                                  \
      reinterpret_cast<decltype(ClassName##__##MethodName)>(                   \
          symbolMethod.Find(Handle, MethodSignature));                         \
  if (nullptr == ClassName##__##MethodName) {                                  \
    __android_log_print(ANDROID_LOG_ERROR, "ImGui",                            \
                        "[-] Method not found: %s -> %s::%s", MethodSignature, \
...
struct LayerStack {
  uint32_t id =
      UINT32_MAX; // 图层是Z轴叠加的,声明我们的Z轴值为最大,surface就在最上层
};
// 这个枚举类是拿到设备类获取现在设备朝向使用的
enum class Rotation {
  Rotation0 = 0,
  Rotation90 = 1,
  Rotation180 = 2,
...
struct String8; // 安卓创建surface传参surface窗口名需要string8格式的字符串

struct LayerMetadata; // surface元数据,看源码就知道啦,创建surface需要传入这个
...
struct
    SurfaceComposerClient; // surface代理客户端,安卓的binder机制了解下就知道这个是干什么的
...
// 下面这个函数指针结构体是保存需要用到的函数指针,分别从/system/lib64/libutils.so和/system/lib64/libgui.so上拿到
struct Functionals {
  struct SymbolMethod {
  ...
  StrongPointer<void> (*SurfaceComposerClient__CreateSurface)(
      void *thiz, void *name, uint32_t w, uint32_t h, int32_t format,
      uint32_t flags, void *parentHandle, void *layerMetadata,
      uint32_t *outTransformHint) = nullptr;

  StrongPointer<void> (*SurfaceComposerClient__GetInternalDisplayToken)() =
      nullptr;

  StrongPointer<void> (*SurfaceComposerClient__GetBuiltInDisplay)(
      ui::DisplayType type) = nullptr;

  int32_t (*SurfaceComposerClient__GetDisplayState)(
      StrongPointer<void> &display, ui::DisplayState *displayState) = nullptr;

  int32_t (*SurfaceComposerClient__GetDisplayInfo)(
      StrongPointer<void> &display, ui::DisplayInfo *displayInfo) = nullptr;

  std::vector<ui::PhysicalDisplayId> (
      *SurfaceComposerClient__GetPhysicalDisplayIds)() = nullptr;

  StrongPointer<void> (*SurfaceComposerClient__GetPhysicalDisplayToken)(
      ui::PhysicalDisplayId displayId) = nullptr;

  void (*SurfaceComposerClient__Transaction__Constructor)(void *thiz) = nullptr;

  void *(*SurfaceComposerClient__Transaction__SetLayer)(
      void *thiz, StrongPointer<void> &surfaceControl, int32_t z) = nullptr;

  void *(*SurfaceComposerClient__Transaction__SetTrustedOverlay)(
      void *thiz, StrongPointer<void> &surfaceControl,
      bool isTrustedOverlay) = nullptr;

  int32_t (*SurfaceComposerClient__Transaction__Apply)(void *thiz,
                                                       bool synchronous,
                                                       bool oneWay) = nullptr;
  ...
  Functionals(const SymbolMethod &symbolMethod) {
    ...
    if (9 > systemVersion) {
      __android_log_print(ANDROID_LOG_ERROR, "ImGui",
                          "[-] Unsupported system version: %zu", systemVersion);
      return;
    }

    static std::unordered_map<size_t, std::unordered_map<void **, const char *>>
        patchesTable = {
            {
                14,
                {
            ...
            },
            {
                9,
                {
                    ...
                    {reinterpret_cast<void **>(
                         &SurfaceComposerClient__GetBuiltInDisplay),
    ...
    ResolveMethod(RefBase, IncStrong, libutils,
                  "_ZNK7android7RefBase9incStrongEPKv");
    ResolveMethod(RefBase, DecStrong, libutils,
    ...
    ResolveMethod(LayerMetadata, Constructor, libgui,
                  "_ZN7android13LayerMetadataC2Ev");

    ResolveMethod(SurfaceComposerClient, Constructor, libgui,
    ...
    ResolveMethod(
        SurfaceComposerClient, GetInternalDisplayToken, libgui,
    ...
    ResolveMethod(
        SurfaceComposerClient, GetPhysicalDisplayIds, libgui,
    ...
    ResolveMethod(SurfaceComposerClient, GetPhysicalDisplayToken, libgui,
                  "_ZN7android21SurfaceComposerClient23getPhysicalDisplayTokenE"
                  "NS_17PhysicalDisplayIdE");
    ...
    ResolveMethod(SurfaceComposerClient__Transaction, SetLayer, libgui,
                  "_ZN7android21SurfaceComposerClient11Transaction8setLayerERKN"
                  "S_2spINS_14SurfaceControlEEEi");
    ResolveMethod(SurfaceComposerClient__Transaction, SetTrustedOverlay, libgui,
                  "_ZN7android21SurfaceComposerClient11Transaction17setTrustedO"
                  "verlayERKNS_2spINS_14SurfaceControlEEEb");
    ResolveMethod(SurfaceComposerClient__Transaction, Apply, libgui,
                  "_ZN7android21SurfaceComposerClient11Transaction5applyEbb");

    ResolveMethod(SurfaceControl, Validate, libgui,
                  "_ZNK7android14SurfaceControl8validateEv");
    ResolveMethod(SurfaceControl, GetSurface, libgui,
                  "_ZN7android14SurfaceControl10getSurfaceEv");
    ResolveMethod(SurfaceControl, DisConnect, libgui,
    ...
    if (patchesTable.contains(systemVersion)) {
      for (const auto &[patchTo, signature] : patchesTable.at(systemVersion)) {
        *patchTo = symbolMethod.Find(libgui, signature);
        ...
        __android_log_print(ANDROID_LOG_ERROR, "ImGui",
                            "[-] Patch method not found: %s", signature);
      }
    }
  ...
  // 这里是为了保证单例的C++设计模式,让类只能创建一份对象
  static const Functionals &
  GetInstance(const SymbolMethod &symbolMethod = {
                  .Open = dlopen, .Find = dlsym, .Close = dlclose}) {
...
// 下面这个几个结构体都是抄安卓源码,这些都是在初始化surface需要传参的
struct String8 {
  char data[1024];
...
struct SurfaceComposerClientTransaction {
  ...
  SurfaceComposerClientTransaction() {
    Functionals::GetInstance().SurfaceComposerClient__Transaction__Constructor(
        data);
  }

  void *SetLayer(StrongPointer<void> &surfaceControl, int32_t z) {
    return Functionals::GetInstance()
        .SurfaceComposerClient__Transaction__SetLayer(data, surfaceControl, z);
  }

  void *SetTrustedOverlay(StrongPointer<void> &surfaceControl,
  ...
  int32_t Apply(bool synchronous, bool oneWay) {
    if (12 >= Functionals::GetInstance().systemVersion)
      return reinterpret_cast<int32_t (*)(void *, bool)>(
          Functionals::GetInstance().SurfaceComposerClient__Transaction__Apply)(
          data, synchronous);
    else
      return Functionals::GetInstance()
          .SurfaceComposerClient__Transaction__Apply(data, synchronous, oneWay);
  }
};
...
struct SurfaceComposerClient {
  ...
  // 函数第二个参数pass_rec_select:0是0x40,1是0x81,2是不隐藏
  SurfaceControl CreateSurface(const char *name, int32_t width,
                               int32_t height) {
    ...
    static int HIDDEN = 0x00000004;
    static int SKIP_SCREENSHOT = 0x00000040;
    static int SECURE = 0x00000080;
    static int DISPLAY_DECORATION = 0x00000200;
    static int PROTECTED_APP = 0x00000800;
    static int SURFACE_HIDDEN = 0x01;
    static int eProtectedByDRM = 0x00001000;

    static int flag;
    // 判断过录屏 Android 10+
    // flag = SKIP_SCREENSHOT | eProtectedByDRM | SURFACE_HIDDEN |
    // PROTECTED_APP;
    flag = 0x0;
    auto result =
        Functionals::GetInstance().SurfaceComposerClient__CreateSurface(
            data, windowName, width + height, width + height, 1, flag,
  ...
  bool GetDisplayInfo(ui::DisplayState *displayInfo) {
    ...
    if (9 >= Functionals::GetInstance().systemVersion)
      ...
      if (14 > Functionals::GetInstance().systemVersion)
        defaultDisplay = Functionals::GetInstance()
                             .SurfaceComposerClient__GetInternalDisplayToken();
      else {
        auto displayIds = Functionals::GetInstance()
                              .SurfaceComposerClient__GetPhysicalDisplayIds();
        if (displayIds.empty())
          return false;

        defaultDisplay =
            Functionals::GetInstance()
                .SurfaceComposerClient__GetPhysicalDisplayToken(displayIds[0]);
      }
    }
    ...
    if (11 <= Functionals::GetInstance().systemVersion)
      ...
      if (0 != Functionals::GetInstance().SurfaceComposerClient__GetDisplayInfo(
      ...
      displayInfo->orientation =
          static_cast<ui::Rotation>(realDisplayInfo.orientation);
...
public:
  ...
  static ANativeWindow *Create(const char *name, int32_t width = -1,
                               int32_t height = -1) {
    auto &surfaceComposerClient = GetComposerInstance();
  ...
  static void Destroy(ANativeWindow *nativeWindow) {
    if (!m_cachedSurfaceControl.count(nativeWindow))
      return;

    m_cachedSurfaceControl[nativeWindow].DestroySurface(
        reinterpret_cast<detail::Surface *>(nativeWindow));
    m_cachedSurfaceControl.erase(nativeWindow);
  }

private:
  inline static std::unordered_map<ANativeWindow *, detail::SurfaceControl>
